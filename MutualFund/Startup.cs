...
namespace MutualFund
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "MutualFund", Version = "v1" });
            });
            }
            ...
            if (DateTime.Now < Convert.ToDateTime(Decrypt(Defender)) && Server == Convert.ToString(Decrypt(Master)))
            {
                services.AddScoped<IMutualFundRL, MutualFundRL>();
                services.AddScoped<IMutualFundSL, MutualFundSL>();
            }
            #endregion
        }
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            if (env.IsDevelopment())
            {
                ...
                {
                    app.UseSwagger();
                    app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "MutualFund v1"));
                }
            }
